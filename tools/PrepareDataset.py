...
gp_num = 12
road_ids = [
    276183, 276184, 275911, 275912, 276240, 276241, 276264, 276265, 276268,
    276269, 276737, 276738
]
neighbour_info = {
...
def devide_train_TTI_by_roadid():
    ...
    for road_id in set(df["id_road"]):
        ...
        save_csv.to_csv("./train/processed/devided_train_TTI/" + str(road_id) +
                        ".csv",
                        header=True,
                        index=False)
...
def check_and_fill_nan():
    ...
    date_range1 = pd.date_range(start="2019-01-01 00:00:00",
                                end="2019-03-31 23:50:00",
                                freq="10T")
    date_range2 = pd.date_range(start="2019-10-01 00:00:00",
                                end="2019-12-21 23:50:00",
                                freq="10T")

    for road_id in road_ids:
        df = pd.read_csv("./train/processed/devided_train_TTI/" +
                         str(road_id) + ".csv")
        ...
        new_df.to_csv("./train/processed/devided_train_TTI/" + str(road_id) +
                      ".csv",
                      header=True,
                      index=True)
...
def prepare_train_set_group12():
    ...
    for road_id in road_ids:
        ...
        save_csv.to_csv("./train/processed/to_train/train_" + str(road_id) +
                        ".csv",
                        mode="w",
                        header=False,
                        index=False)
...
def add_neighbour_info():
    ...
    for road_id in road_ids:
        ...
        df1 = pd.read_csv(train_csv_path + "train_" + str(road_id) + ".csv",
                          index_col=False,
                          header=None)
        neighbour_dfs = [
            pd.read_csv(train_csv_path + "train_" + str(i) + ".csv",
                        index_col=False,
                        header=None) for i in neighbour_info[road_id]
        ]
        neighbour_cnt = 0
        ...
        df1.to_csv(train_csv_path + "train_" + str(road_id) + ".csv",
                   mode="w",
                   index=False,
                   header=False)
