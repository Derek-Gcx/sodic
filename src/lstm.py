...
def run(road_index):
    ...
    # 初始化网络和优化器
    net = LSTM(input_size, output_size, True)
    para = net.parameters()
    ...
    def train(epoches):
        ...
        for epoch in range(epoches):
            ...
            if epoch == epoches-1:
                torch.save(net.state_dict(), './out/group_'+str(road_index)+'_LSTM.pth')
    ...
    def para_switch(road_id):
        ...
        net.load_state_dict(torch.load('./out/group_'+g+'_LSTM.pth'))
    ...
    def test():
        ...
        with torch.no_grad():
            ...
            for line in open("././train/processed/test_feature/ToPredict.csv"):
                ...
                count += 1
                ipt_tensor = torch.from_numpy(np.array(line)).reshape(1, 6, input_size).double()
                result += net(ipt_tensor).reshape(-1).numpy().tolist()
    ...
    train(200)
    net.load_state_dict(torch.load('./out/group_'+str(road_index)+'_LSTM.pth'))
    valid()
    # figure()
    if road_id == 11:
        test()
